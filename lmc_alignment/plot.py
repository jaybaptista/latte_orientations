...
ax_i.plot(m12i_time, np.array(m12i_data["disk"])[m12i_m], c=disk_col, label="Stellar Disk ($R_{*,90}$)", lw=disk_lw, ls=disk_ls)
ax_i.plot(m12i_time, np.array(m12i_data["5.disk"])[m12i_m], c=disk5_col, label="5$ \\times R_{*,90}$", lw=disk5_lw, ls=disk5_ls)
ax_i.plot(m12i_time, np.array(m12i_data["virial"])[m12i_m], c=vir_col, label="Virial Radius $(R_{200})$", lw=vir_lw, ls=vir_ls)

ax_f.plot(m12f_time, np.array(m12f_data["disk"])[m12f_m], c=disk_col, lw=disk_lw, ls=disk_ls)
ax_f.plot(m12f_time, np.array(m12f_data["5.disk"])[m12f_m], c=disk5_col, lw=disk5_lw, ls=disk5_ls)
ax_f.plot(m12f_time, np.array(m12f_data["virial"])[m12f_m], c=vir_col, lw=vir_lw, ls=vir_ls)

ax_m.plot(m12m_time, np.array(m12m_data["disk"])[m12m_m], c=disk_col, ls=disk_ls, lw=disk_lw)
ax_m.plot(m12m_time, np.array(m12m_data["5.disk"])[m12m_m], c=disk5_col, ls=disk5_ls, lw=disk5_lw)
ax_m.plot(m12m_time, np.array(m12m_data["virial"])[m12m_m], c=vir_col, ls=vir_ls, lw=vir_lw)

ax_w.plot(m12w_time, np.array(m12w_data["disk"])[m12w_m], c=disk_col, ls=disk_ls, lw=disk_lw)
ax_w.plot(m12w_time, np.array(m12w_data["5.disk"])[m12w_m], c=disk5_col, ls=disk5_ls, lw=disk5_lw)
ax_w.plot(m12w_time, np.array(m12w_data["virial"])[m12w_m], c=vir_col, ls=vir_ls, lw=vir_lw)
